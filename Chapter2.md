...
`-1L < 1U`, because `1U`, which is an `unsigned int`, is promoted to a `signed
long`. But `-1L > 1UL` because `-1L` is promoted to `unsigned long` and thus
...
sets the last six bits of `x` to zero. Note that `x & ~077` is independent of
...
As an example, the function bitcount counts the number of 1-bits in its integer
argument.
...
We have already seen that the assignment statement has a value and can occur in
...
the expression are updated. One unhappy situation is typified by the statement
